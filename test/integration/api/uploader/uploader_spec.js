...
describe("uploader", function () {
  ...
  after(function () {
    ...
    return Q.allSettled([
      ...
      !cloudinary.config().keep_test_products ? cloudinary.api.deleteResourcesByTag(TEST_TAG,
        {
          resource_type: "video"
        }) : void 0
    ]);
  ...
  it('should allow upload with url safe base64 in overlay', function () {
    ...
    return cloudinary.uploader.upload(baseImageUrl, options)
      .then((result) => {
  ...
  describe("remote urls ", function () {
    ...
    it("should send s3:// URLs to server", function () {
      cloudinary.uploader.upload("s3://test/1.jpg", {
        tags: UPLOAD_TAGS
      });
    ...
    it("should send gs:// URLs to server", function () {
      cloudinary.uploader.upload("gs://test/1.jpg", {
        tags: UPLOAD_TAGS
      });
    ...
    it("should send ftp:// URLs to server", function () {
      cloudinary.uploader.upload("ftp://example.com/1.jpg", {
        tags: UPLOAD_TAGS
      });
  ...
  describe("rename", function () {
    ...
    it("should successfully rename a file", function () {
      return uploadImage().then(function (result) {
        return cloudinary.uploader.rename(result.public_id, result.public_id + "2").then(function () {
          return result.public_id;
        });
      }).then(function (public_id) {
        return cloudinary.api.resource(public_id + "2");
      });
    });
    it("should not rename to an existing public_id", function () {
      return Promise.all([uploadImage(), uploadImage()]).then(function (results) {
        return cloudinary.uploader.rename(results[0].public_id, results[1].public_id);
      }).then(function () {
        expect().fail();
    ...
    return context(":invalidate", function () {
      ...
      it("should pass the invalidate value in rename to the server", function () {
        cloudinary.uploader.rename("first_id", "second_id", {
          invalidate: true
        });
  ...
  it("should support `async` option in explicit api", function () {
    return cloudinary.uploader.explicit("sample", {
      type: "facebook",
      eager: [
        {
          crop: "scale",
  ...
  it("should successfully upload stream", function (done) {
    ...
    stream = cloudinary.uploader.uploadStream({
      tags: UPLOAD_TAGS
  ...
  describe("tags", function () {
    ...
    it("should add tags to existing resources", function () {
      return uploadImage().then(function (result) {
        ...
        return cloudinary.uploader.addTag("tag1", [firstId, secondId]).then(function () {
          return [firstId, secondId];
        });
      }).then(function ([firstId, secondId]) {
        return cloudinary.api.resource(secondId).then(function (r1) {
          expect(r1.tags).to.contain("tag1");
        }).then(function () {
          return [firstId, secondId];
        });
      }).then(function ([firstId, secondId]) {
        return cloudinary.uploader.removeAllTags([firstId, secondId, 'noSuchId']).then(function (result) {
    ...
    it("should keep existing tags when adding a new tag", function () {
      return uploadImage().then(function (result) {
        return cloudinary.uploader.addTag("tag1", result.public_id).then(function () {
          return result.public_id;
        });
      }).then(function (publicId) {
        return cloudinary.uploader.addTag("tag2", publicId).then(function () {
          return publicId;
        });
      }).then(function (publicId) {
    ...
    it("should replace existing tag", function () {
      return cloudinary.uploader.upload(IMAGE_FILE, {
        ...
        return cloudinary.uploader.replaceTag("tag3Å", public_id).then(function () {
          return public_id;
        });
  ...
  describe("context", function () {
    ...
    it("should add context to existing resources", function () {
      return cloudinary.uploader
        .addContext('alt=testAlt|custom=testCustom', [this.first_id, this.second_id])
        .then(() => cloudinary.uploader.addContext({
          alt2: "testAlt2",
          custom2: "testCustom2"
        }, [this.first_id, this.second_id]))
        .then(() => cloudinary.api.resource(this.second_id))
        .then(({ context }) => {
          ...
          return cloudinary.uploader.removeAllContext([this.first_id, this.second_id, 'noSuchId']);
        }).then(({ public_ids }) => {
          ...
          return cloudinary.api.resource(this.second_id);
    ...
    it("should upload with context containing reserved characters", function () {
      ...
      return cloudinary.uploader.upload(IMAGE_FILE, {
        ...
        return cloudinary.api.resource(result.public_id, {
          context: true
        });
      }).then(function (result) {
  ...
  it("should allow sending face coordinates", function () {
    ...
    return cloudinary.uploader.upload(IMAGE_FILE, {
      ...
      return cloudinary.uploader.explicit(result.public_id, {
        faces: true,
  ...
  it("should support requesting raw conversion", function () {
    return cloudinary.uploader.upload(RAW_FILE, {
      raw_convert: "illegal",
      resource_type: "raw",
      tags: UPLOAD_TAGS
    }).then(function () {
  ...
  describe("upload_chunked", function () {
    ...
    it("should specify chunk size", function (done) {
      return fs.stat(LARGE_RAW_FILE, function (err, stat) {
        cloudinary.uploader.uploadLarge(LARGE_RAW_FILE, {
          chunk_size: 7000000,
    ...
    it("should use file name", function (done) {
      fs.stat(LARGE_RAW_FILE, function (err, stat) {
        return cloudinary.uploader.uploadLarge(LARGE_RAW_FILE, {
          use_filename: true
        }, function (error, result) {
          if (error != null) {
            done(new Error(error.message));
    ...
    it("should support uploading a small raw file", function (done) {
      fs.stat(RAW_FILE, function (err, stat) {
        cloudinary.uploader.uploadLarge(RAW_FILE, {
          tags: UPLOAD_TAGS
        }, function (error, result) {
          if (error != null) {
            done(new Error(error.message));
    ...
    it("should support uploading a small image file", function (done) {
      fs.stat(IMAGE_FILE, function (err, stat) {
        return cloudinary.uploader.uploadChunked(IMAGE_FILE, {
          tags: UPLOAD_TAGS
        }, function (error, result) {
          if (error != null) {
            done(new Error(error.message));
    ...
    it("should support uploading large video files", function () {
      ...
      return Q.denodeify(cloudinary.uploader.uploadChunked)(LARGE_VIDEO, {
        chunk_size: 6000000,
        resource_type: 'video',
    ...
    it("should support uploading based on a url", function (done) {
      this.timeout(TIMEOUT.MEDIUM);
      cloudinary.uploader.uploadLarge("http://cloudinary.com/images/old_logo.png", {
        tags: UPLOAD_TAGS
      }, function (error, result) {
        if (error != null) {
          done(new Error(error.message));
  ...
  it("should reject with promise rejection if disable_promises: false", function (done) {
    ...
    cloudinary.uploader.uploadLarge(EMPTY_IMAGE, { disable_promises: false }, () => {});
  ...
  it("should reject with promise rejection by default", function (done) {
    ...
    cloudinary.uploader.uploadLarge(EMPTY_IMAGE, () => {});
  ...
  it("should reject without promise rejection if disable_promises: true", function (done) {
    ...
    cloudinary.uploader.uploadLarge(EMPTY_IMAGE, { disable_promises: true }, () => {});
  ...
  it("should reject promise if error code is returned from the server", function () {
    return cloudinary.uploader.upload(EMPTY_IMAGE, {
      tags: UPLOAD_TAGS
    }).then(function () {
  ...
  it("should successfully upload with pipes", function (done) {
    ...
    this.timeout(TIMEOUT.LONG);
    upload = cloudinary.uploader.uploadStream({
      tags: UPLOAD_TAGS
    }, function (error, result) {
      var expected_signature;
      expect(result.width).to.eql(241);
  ...
  it("should fail with http.Agent (non secure)", function () {
    this.timeout(TIMEOUT.LONG);
    expect(cloudinary.uploader.uploadStream).withArgs({
  ...
  it("should pass `accessibility_analysis` option to the server", function () {
    return helper.provideMockObjects((mockXHR, writeSpy, requestSpy) => {
      cloudinary.uploader.upload(IMAGE_FILE, { accessibility_analysis: true });
  ...
  describe("explicit", function () {
    ...
    it("should support metadata", function () {
      const metadata_fields = METADATA_SAMPLE_DATA;
      cloudinary.uploader.explicit("cloudinary", { metadata: metadata_fields });
    ...
    it("should pass `accessibility_analysis` to server", function () {
      cloudinary.uploader.explicit("cloudinary", { accessibility_analysis: true });
  ...
  describe("update_metadata", function () {
    it("should update metadata of existing resources", function () {
      ...
      return helper.provideMockObjects(function (mockXHR, writeSpy, requestSpy) {
        cloudinary.uploader.updateMetadata(metadata_fields, public_ids);
        sinon.assert.calledWith(requestSpy, sinon.match({
          method: sinon.match("POST")
        }));
  ...
  describe(":ocr", function () {
    ...
    it("should support requesting ocr when uploading", async function () {
      ...
      const result = await UPLOADER.upload(IMAGE_FILE, {ocr: ocrType, tags: [TEST_TAG]});
    ...
    it("should support ocr parameter in explicit", async function () {
      ...
      // Upload an image
      const uploadResult = await UPLOADER.upload(IMAGE_FILE, {
        tags: [TEST_TAG]
      });

      // Call explicit on the uploaded image with ocr parameter
      const explicitResult = await UPLOADER.explicit(uploadResult.public_id, {
        ocr: ocrType,
        "tags": [TEST_TAG],
        type: "upload"
      });
  ...
  describe("structured metadata fields", function () {
    ...
    before(function () {
      return cloudinary.api.addMetadataField({
    ...
    after(function () {
      return cloudinary.api.deleteMetadataField(METADATA_FIELD_UNIQUE_EXTERNAL_ID)
        .finally(function () {});
    });
    ...
    it("should be set when calling explicit with metadata", function () {
      return uploadImage({
        ...
        .then(result => cloudinary.uploader.explicit(result.public_id, {
          type: "upload",
          metadata: metadata_fields
        }))
        .then((result) => {
    ...
    it("should be updatable with uploader.update_metadata on an existing resource", function () {
      ...
      return uploadImage({
        ...
        .then((result) => {
          ...
          return cloudinary.uploader.updateMetadata(metadata_fields, [publicId]);
        })
        .then((result) => {
          expect(result).not.to.be.empty();
    ...
    it("should be updatable with uploader.update_metadata on multiple existing resources", function () {
      ...
      return Q.allSettled(
        ...
        return cloudinary.uploader.updateMetadata(metadata_fields, [resource_1.public_id, resource_2.public_id]);
      })
        .then((result) => {
  ...
  it('should add the eval parameter to an uploaded asset', async () => {
    const result = await UPLOADER.upload(IMAGE_FILE, {
      tags: [TEST_TAG],
      eval: TEST_EVAL_STR
    });
  ...
  describe("proxy support", function () {
    ...
    it("should support proxy for upload calls", function () {
      ...
      UPLOADER.upload(IMAGE_FILE, {"tags": [TEST_TAG]});
    ...
    it("should prioritize custom agent", function () {
      ...
      UPLOADER.upload(IMAGE_FILE, {"tags": [TEST_TAG], agent: custom_agent});
      sinon.assert.calledWith(mocked.request, sinon.match(
    ...
    it("should support api_proxy as options key", function () {
      cloudinary.config({});
      UPLOADER.upload(IMAGE_FILE, {"tags": [TEST_TAG], api_proxy: proxy});
      sinon.assert.calledWith(mocked.request, sinon.match(
        arg => arg.agent instanceof ProxyAgent
      ));
    });
