...
function uploadChunkedStream(options = {}, callback) {
  ...
  chunker.on('ready', function (buffer, is_last, done) {
    ...
    let finished_part = function (error, result) {
      const errorOrLast = (error && error.message != null) || is_last;
...
function callApi(action, options, get_params, _callback) {
