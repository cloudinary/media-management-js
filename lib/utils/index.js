...
exports.USER_AGENT = `CloudinaryMMNodeJS/${exports.VERSION} (Node ${process.versions.node})`;
...
function updateable_resource_params(options, params = {}) {
  ...
  if (options.use_asset_id != null) {
    params.use_asset_id = options.use_asset_id;
  }
  return params;
}
...
function apiSignRequest(params_to_sign, api_secret) {
...
function sign_request(params, options = {}) {
  ...
  params.signature = exports.apiSignRequest(params, apiSecret);
  params.api_key = apiKey;
  return params;
}

function webhookSignature(data, timestamp, options = {}) {
...
function verifyNotificationSignature(body, timestamp, signature, valid_for = 7200) {
  ...
  const payload_hash = utils.webhookSignature(body, timestamp, {
...
function privateDownloadUrl(public_id, format, options = {}) {
  let params = exports.sign_request({
    ...
    use_asset_id: options.use_asset_id
  }, options);
 ...
 * @deprecated Replaced by {downloadZipUrl} that uses the more advanced and robust archive generation and download API
 */

function zipDownloadUrl(tag, options = {}) {
  let params = exports.sign_request({
    timestamp: options.timestamp || exports.timestamp(),
    tag: tag,
 ...
 * @returns {string }
 */
function downloadBackedupAsset(asset_id, version_id, options = {}) {
  let params = exports.sign_request({
    timestamp: options.timestamp || exports.timestamp(),
    asset_id: asset_id,
 ...
 * @param options
 */
function apiDownloadUrl(action, params, options) {
 ...
 */
function downloadArchiveUrl(options = {}) {
  const params = exports.archive_params(merge(options, {
    mode: "download"
  }))
  return apiDownloadUrl("generate_archive", params, options)
}

/**
 * Returns a URL that when invokes creates an zip archive and returns it.
 * @see downloadArchiveUrl
 */

function downloadZipUrl(options = {}) {
  return exports.downloadArchiveUrl(merge(options, {
    target_format: "zip"
  }));
}
 ...
 */
function downloadFolder(folder_path, options = {}) {
...
function adapter(callback, options) {
  ...
  return callback_adapter(callback);
}

function callback_adapter(callback){
...
exports.apiDownloadUrl = apiDownloadUrl;
...
exports.apiSignRequest = apiSignRequest;
...
exports.privateDownloadUrl = privateDownloadUrl;
exports.zipDownloadUrl = zipDownloadUrl;
exports.downloadArchiveUrl = downloadArchiveUrl;
exports.downloadBackedupAsset = downloadBackedupAsset;
exports.downloadZipUrl = downloadZipUrl;
exports.callback_adapter = callback_adapter;
